		...
		return this.each(function(){
			...
			if($this.val() != ""){
				...
				$.each(items,function(key,item){
					main_wap.append(createTimeline(item.frame,item.title,item.description,item.date));
					 $(".ntm-items").sortable();
				});
			}
			...
			function nootimelineUpdate(){
				...
				items.each(function(index,element){
					var that = $(this),
						item = {'frame':that.find("#timeline_frame").val(),'title':that.find("#timeline_title").val(),'description':that.find("#timeline_description").val(),'date':that.find("#timeline_date").val()};
					if (Object.keys(item).length) config[index] = item;
				});
				$this.val(htmlspecialchars(JSON.stringify(config)));
			};
			
			function createTimeline(frame,title,description,date){
				...
				var html = '<div class="well well-small ntm-item"><button onclick="nootimelineRemove(this);" class="btn btn-small" id="nootimeline_remove" type="button"><i class="icon-remove icon-white" title="Remove"></i>Remove</button>'
								...
								+'<div class="control-group">'
									+'<div class="control-label">'
										+'<label for="timeline_date">Timeline Date</label>'
									+'</div>'
									+'<div class="controls">'
										+'<input type="text" id="timeline_date" name="timeline_date" value="'+date+'">'
									+'</div>'
								+'</div>'
							+'</div>';
				return html;
