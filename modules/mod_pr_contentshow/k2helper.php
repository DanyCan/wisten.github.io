...
abstract class modPRCONTENTK2Helper {
			...
			if (count($items)) {
				...
				foreach ($items as $item) {
					...
					if (! empty($item->created_by_alias)) {
						...
						$query1 = "SELECT * FROM #__k2_users where userID='$author_id'";
						$count1="";
						$db->setQuery($query1, 0, $count1);
			            $items1 = $db->loadObjectList();
						if (count($items1)) {
				         foreach ($items1 as $item1) {
				               $item->author_image=$item1->image;
					...
					$item->comment		= '<a class="ns2-comments" href="' . $item->link . '#itemCommentsAnchor">' . JText::_('') . ' (' . $model->countItemComments($item->id) . ')</a>';
